...
const SchoolList = () => {
  ...
  const handleDelete = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`${API_NODE_URL}school/delete?id=${event._id}`, {
        method: "GET",
        headers: {
          ...
          <table className="w-full table-auto border-collapse">
            <thead>
              <tr>
                ...
                <th className="border px-4 py-2">School Code</th>
                <th className="border px-4 py-2">Established In</th>
              ...
              {newsAndEvents.length > 0 ? (
                newsAndEvents.map((event) => (
                  <tr key={event._id}>
                    ...
                    <td className="border px-4 py-2">{event?.schoolCode}</td>
                    <td className="border px-4 py-2">{event.schoolType}</td>
                    <td className="border px-4 py-2">
                      {event.yearEstablished}
                    </td>
                    ...
                    <td className="border px-4 py-2">
                      <div className="flex justify-center space-x-2">
                        <button
                          onClick={() =>
                            router.push(`/admin/edit-school?_id=${event?._id}`)
                          }
