...
const MetaDataList = () => {
  ...
  const handleDelete = async (pageid) => {
    if (window.confirm("Are you sure you want to delete this event?")) {
      try {
        const response = await fetch(`${API_NODE_URL}meta/delete`, {
          method: "POST",
      ...
      <div className="overflow-y-auto h-[85vh] bg-white rounded-lg shadow-md p-6">
        {loading ? (
          ...
          <table className="w-full table-auto border-collapse">
            <thead>
              <tr>
                <th className="border px-4 py-2 text-left">Meta Title</th>
            ...
            <tbody>
              {MetaData.length > 0 ? (
                MetaData.map((event) => (
                  <tr key={event._id}>
                    <td className="border px-4 py-2">{event.metatitle}</td>
                    <td className="border px-4 py-2">{event.metaDescription}</td>
                    <td className="border px-4 py-2">{event.metaKeywords}</td>
                    <td className="border px-4 py-2">{event.path}</td>
                    <td className="border px-4 py-2">{event.pageName}</td>
                    <td className="border px-4 py-2">
                      <div className="flex justify-center space-x-2">
                        ...
                        <button
                          onClick={() => handleDelete(event.pageid)}
