  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `${API_NODE_URL}slug/getbytype?type=News,Event`
      );
              ...
              {newsAndEvents.length > 0 ? (
                newsAndEvents.map((event) => (
                  <tr key={event.id}>
                    ...
                    <td className="border px-4 py-2" dangerouslySetInnerHTML={{ __html: event.shortdesc }}></td>
                    <td className="border px-4 py-2">
                    ...
                    <td className="border px-4 py-2">
                      <div className="flex justify-center space-x-2">
                        <button
                          onClick={() =>
                            router.push(
                              `/admin/edit-news-and-event?page_id=${event?.page_id}`
                            )
                          }
